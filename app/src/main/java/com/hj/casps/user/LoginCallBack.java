...
public abstract class LoginCallBack<T> extends AbsCallback<T> {
    ...
    public T convertSuccess(Response response) throws Exception {
        ...
        if (typeArgument == Void.class) {
            ...
            return (T) simpleResponse.toLoginContext();
        } else if (rawType == LoginContextBean.class) {
            //有数据类型，表示有data
            LoginContextBean loginSimpleGain = Convert.fromJson(jsonReader, type);
            response.close();
            int code = loginSimpleGain.return_code;
            //这里的0是以下意思
            //一般来说服务器会和客户端约定一个数表示成功，其余的表示失败，这里根据实际情况修改
            if (code == 0) {
                return (T) loginSimpleGain;
            } else if (code == 101) {
