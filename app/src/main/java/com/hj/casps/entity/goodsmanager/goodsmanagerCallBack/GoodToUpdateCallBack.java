...
public abstract class GoodToUpdateCallBack<T> extends AbsCallback<T> {
    ...
    public T convertSuccess(Response response) throws Exception {
        ...
        if (typeArgument == Void.class) {
            ...
            return (T) simpleResponse.toGoodtoUpdateGain();
        } else if (rawType == GoodtoUpdateGain.class) {
            //有数据类型，表示有data
            GoodtoUpdateGain goodtoUpdateGain = Convert.fromJson(jsonReader, type);
            response.close();
            int code = goodtoUpdateGain.return_code;
            //这里的0是以下意思
            //一般来说服务器会和客户端约定一个数表示成功，其余的表示失败，这里根据实际情况修改
            if (code == 0) {
                return (T) goodtoUpdateGain;
            } else if (code == 101) {
