...
//                            mHandler.sendEmptyMessage(Constant.HANDLERTYPE_0);
                            ...
                            ArrayList<ProtocalProductItem.GoodLevelEntity> dataList = generateData2();
                            //如果数据为空，就跳到添加数据页面
    ...
    //初始化菜单数据
    private ArrayList<ProtocalProductItem.GoodLevelEntity> generateData2() {
        ArrayList<ProtocalProductItem.GoodLevelEntity> res = new ArrayList<>();
        for (int i = 0; i < categoryList.size(); i++) {
            NoteEntity noteEntity = categoryList.get(i);
            ProtocalProductItem.GoodLevelEntity lv0 = new ProtocalProductItem.GoodLevelEntity(
     ...
     * @return
     */
    private void resolveData(ProtocalProductItem.GoodLevelEntity lv0, NoteEntity noteEntity) {
        if (noteEntity.getNodes() != null && noteEntity.getNodes().size() > 0) {
            for (int i = 0; i < noteEntity.getNodes().size(); i++) {
                ...
                ProtocalProductItem.GoodLevelEntity goodEntity = new ProtocalProductItem.GoodLevelEntity(
